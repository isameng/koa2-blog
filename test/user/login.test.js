...
//   headers: {
//     'set-cookie': [
//       'blog.sid=voZK39rCfCZI9-V6LPwZcgaDp7oXLCOi; path=/; expires=Fri, 29 Jan 2021 09:11:57 GMT; httponly',
//       'blog.sid.sig=90-3h3tNqJqHhBWGdzyHB0OS8K0; path=/; expires=Fri, 29 Jan 2021 09:11:57 GMT; httponly'
//     ],
//   },
...
test('修改密码，应该成功', async () => {
  const res = await server.patch('/api/user/changePassword').send({
    password, newPassword:`p_${Date.now()}`
  }).set('cookie', COOKIE);
  expect(res.body.errno).toBe(0);
})
...
//再次查询用户
test('删除用户之后，再次查询注册的用户名，应该不存在', async () => {
  const res = await server.post('/api/user/isExist').send({
    userName
  });
  expect(res.body.errno).not.toBe(0);
